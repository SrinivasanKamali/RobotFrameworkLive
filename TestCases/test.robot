...
TC001_Checking the testing application with valid data
    ...
    ${enddate}=        dataval.Get Row    ${file}    ${sheet}    ${i}    to
    #${file1_val}=      dataval.Get Row    ${file}    ${sheet}    ${i}    file1
    #${file2_val}=      dataval.Get Row    ${file}    ${sheet}    ${i}    file2
    ${file1_msg}=      dataval.Get Row    ${file}    ${sheet}    ${i}    msg1
    ${file2_msg}=      dataval.Get Row    ${file}    ${sheet}    ${i}    msg2
    Enter the information   ${name_val}  ${title_val}  ${email_val}   ${cell}   ${address_val}  ${gender_val}  ${country_val}  ${colors_val}   ${list_val}  ${date1}   ${date2}  ${fromdate}    ${enddate}
